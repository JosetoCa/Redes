"""
Precomputa JPS para MNIST y guarda como TFRecord(s) usando una llave de encriptación compartida.
...
seed_obj = 1234
seed_key = 9999
out_dir = "data_jps_singlekey" # Carpeta con datos
...
def compute_jps_with_shared_key(img, pm_obj, pm_key, N=160, offset=40, sigma=10.0):
    """
...
def write_sharded_tfrecord(prefix, images, pm_obj, pm_key, N, offset, sigma, out_dir, shards=4):
    """
    ...
    for shard_idx, cnt in enumerate(counts):
        ...
        for i in tqdm(range(start, end), desc=f"shard {shard_idx}", unit="sample"):
            ...
            jps = compute_jps_with_shared_key(images[i], pm_obj, pm_key, N=N, offset=offset, sigma=sigma)
            # Crea el diccionario que se va a escribir el TFRecord
            feat = {
                'jps_raw': _bytes_feature(jps.tobytes()),
                'mnist_raw'  : _bytes_feature(images[i].astype(np.float32).tobytes()),
...
if __name__ == "__main__":
    ...
    # generar llave única (pm_obj y pm_key)
    pm_obj = random_phase_mask(N, seed=seed_obj)
    pm_key = random_phase_mask(N, seed=seed_key)
    # guardar llaves (opcional pero útil)
    np.save(os.path.join(out_dir, "pm_obj.npy"), pm_obj)
    np.save(os.path.join(out_dir, "pm_key.npy"), pm_key)
    # metadata
    meta = dict(N=int(N), offset=int(offset), sigma=float(sigma), seed_obj=int(seed_obj), seed_key=int(seed_key), use_log=False)
    np.save(os.path.join(out_dir, "metadata.npy"), meta)
    print("Saved keys and metadata in", out_dir)

    # escribir TFRecords shardeados
    write_sharded_tfrecord(train_tfrecord_prefix, x_train, pm_obj, pm_key, N, offset, sigma, out_dir, shards=shards)
    write_sharded_tfrecord(test_tfrecord_prefix,  x_test,  pm_obj, pm_key, N, offset, sigma, out_dir, shards=max(1, shards//4))
